    ...
    */
    ...
    while (queue.length > 0) {
        ...
        for (const [nx, ny] of neighbors) {
            ...
            if (isValid(nx, ny, dim) && !visited.has(`${nx},${ny}`) && board[nx][ny]) {
    ...
    while (exploration.length !== 0) {
        ...
        for (const [currentMove, prevDirection] of exploration) {
            ...
            for (let direction of newDirections) {
                ...
                if (isValid(nx, ny, dim) && board[nx][ny] && !visited.has(`${nx},${ny},${direction}`)) {
...
function checkFork(board, move, playerNum) {
    ...
    let edges = getAllEdges(dim).map(edge => new Set(edge.map(([x, y]) => `${x},${y}`)));

    // Determine reachable edges (only count edges connected by player's path)
    let reachableEdges = edges.filter(edge => [...visited].some(point => edge.has(point)));
...
function checkForkAndBridge(board, move) {
    ...
    */
    ...
    const dim = board.length;
    const corners = new Set(getAllCorners(dim).map(([x, y]) => `${x},${y}`));
    const sides = getAllEdges(dim).map(side => new Set(side.map(([x, y]) => `${x},${y}`)));

    // Check for fork
    const reachableEdges = sides.map(side => sideHasReachablePoints(side, visited));
    if (reachableEdges.filter(edge => edge).length >= 3) {
        return [true, "fork"];
    }

    // Check for bridge
    const reachableCorners = [...visited].filter(point => corners.has(point)).length;
    if (reachableCorners >= 2) {
        return [true, "bridge"];
    }
